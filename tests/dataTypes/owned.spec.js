...
describe("Owned data type process check", () => {
  ...
  describe("Every tick", () => {
    ...
    it('should handle intentCategories correctly', async () => {
      ...
      const data = await testHelper1.process(settings)
      const base = data.stats.users[data.opts.username].stats.combined.shards[data.opts.shard]
      expect(base.intents.income.harvest.effect).toBe(20 * 2 / 2)
      expect(base.intents.income.dismantle.effect).toBe(2.5 * 2 / 2)

      expect(base.intents.outcome.repair.effect).toBe(10 * 2 / 2)
      expect(base.intents.outcome.build.effect).toBe(50 * 2 / 2)
      expect(base.intents.outcome.upgradeController.effect).toBe(10 * 2 / 2)
